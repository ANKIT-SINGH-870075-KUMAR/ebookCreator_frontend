...
const CreateBookModal = ({isOpen, onClose, onBookCreated}) => {
    ...
    const handleFinalizeBook = async () => {
        ...
        try {
            ...
            console.log("DEBUG - Attempting to access book ID...");
            
            // Check both possible paths
            const bookIdFromData = response.data._id;
            const bookIdFromBook = response.data.book?._id;
            
            console.log("DEBUG - response.data._id:", bookIdFromData);
            console.log("DEBUG - response.data.book._id:", bookIdFromBook);
            ...
            if (bookId) {
                console.log("DEBUG - Calling onBookCreated with bookId:", bookId);
                onBookCreated(bookId);
            } else {
                console.error("DEBUG - No book ID found in response!");
                toast.error("Book created but ID not found. Please try again.");
            }
            
            onClose();
            resetModal();
        } catch (error) {
            console.log("DEBUG - Book creation failed:", error);
                ...
                {chapters.length === 0 ? (
                    <div className="text-center py-12 px-4 bg-gray-50 rounded-xl">
                        ...
                        <p className="text-gray-500 text-sm">
                            No chapters yet. And one to get started.
                        </p>
                    </div>
                ) : (
